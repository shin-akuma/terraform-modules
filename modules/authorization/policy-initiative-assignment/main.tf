...
# Create a dependency marker that the initiative can reference
resource "terraform_data" "assignment_exists" {
  input = "assignment_created"

  # This ensures the marker is destroyed when assignments are destroyed
  lifecycle {
...
## role assignments for linked scopes e.g. Policy Assignment identity access to centralised Log Analytics Workspaces ##
resource "azurerm_role_assignment" "remediation_linked_scope" {
  for_each                         = var.role_assignment_linked_scope == null ? {} : local.role_definition_names
  scope                            = var.role_assignment_linked_scope
  role_definition_name             = each.value
  principal_id                     = local.assignment.identity[0].principal_id
  skip_service_principal_aad_check = true
}
